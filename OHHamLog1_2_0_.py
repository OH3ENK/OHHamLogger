...
class HamLogger:
    def __init__(self, root):
        ...
        # Kieli-asetus
        self.language = 'suomi'
        self.version = "1.2.0"  # FreeDV + muokkaus lisätty
    ...
    def parse_text_qso(self, line):
        ...
        try:
            ...
            for part in parts:
                ...
                # Modet
                elif part_upper in ['SSB', 'LSB', 'USB', 'CW', 'FM', 'AM', 'FT8', 'FT4', 'RTTY', 'PSK', 'FREEDV']:
            ...
            if 'date_part' in locals() and 'time_part' in locals():
                try:
                    ...
                    qso_data['timestamp'] = datetime.datetime.now(datetime.UTC).strftime('%Y-%m-%d %H:%M:%S')
            else:
                qso_data['timestamp'] = datetime.datetime.now(datetime.UTC).strftime('%Y-%m-%d %H:%M:%S')
    ...
    def show_settings_dialog(self, show_station=False, show_other=False):
        ...
        if show_station:
            ...
            mode_combo['values'] = ['SSB', 'LSB', 'USB', 'CW', 'FM', 'AM', 'FT8', 'FT4', 'RTTY', 'FreeDV']
        ...
        if show_other:
            ...
            theme_combo['values'] = ['oletus', 'yö', 'retro', 'elegantti', 'klassinen amatööri']
    ...
    def update_language(self):
        ...
        if self.language == 'english':
            ...
            self.texts = {
                ...
                'help': "Ohje",
                'qso_log': "QSO Loki",
                'input': "Syöttö  (RST järjestys: annettu saatu)",
                ...
                'edit_entry': "Muokkaa merkintää",
                'delete_entry': "Poista merkintä",
                'edit_qso': "Muokkaa QSO:ta"
            }
    ...
    def apply_theme(self):
        ...
        if theme == 'yö':
            ...
            # QSO-loki Text widget - vaalea tausta, musta/punainen teksti
            if hasattr(self, 'log_text'):
                self.log_text.config(bg='#FFFFFF', fg='#000000', insertbackground='#000000')
                self.log_text.tag_configure("duplicate", foreground="#ff0000", font=('Courier New', 9, 'bold'))
                self.log_text.tag_configure("normal", foreground="#000000")
        
        elif theme == 'retro':
            # Retro - tumma tausta, vihreä teksti
            self.root.configure(bg='#000000')
            style = ttk.Style()
            style.theme_use('clam')
            style.configure('TFrame', background='#000000')
            style.configure('TLabel', background='#000000', foreground='#00ff00')
            style.configure('TButton', background='#000000', foreground='#00ff00')
            ...
            style.configure('TEntry', fieldbackground='#000000', foreground='#00ff00', insertcolor='#00ff00')
            style.map('TEntry', 
                     fieldbackground=[('readonly', '#000000'), ('disabled', '#000000')],
                     foreground=[('readonly', '#00ff00'), ('disabled', '#00ff00')])
            self.root.option_add('*Menu.background', '#000000')
            self.root.option_add('*Menu.foreground', '#00ff00')
            
            # QSO-loki Text widget - musta tausta, vihreä/punainen teksti
            if hasattr(self, 'log_text'):
                self.log_text.config(bg='#000000', fg='#00ff00', insertbackground='#00ff00')
                self.log_text.tag_configure("duplicate", foreground="#ff0000", font=('Courier New', 9, 'bold'))
                self.log_text.tag_configure("normal", foreground="#00ff00")
            ...
            style = ttk.Style()
            style.theme_use('clam')
            style.configure('TFrame', background='#2a3b5a')
            style.configure('TLabel', background='#2a3b5a', foreground='#e0e0e0')
            style.configure('TButton', background='#2a3b5a', foreground='#e0e0e0')
            style.configure('TLabelframe', background='#2a3b5a', foreground='#e0e0e0')
            style.configure('TLabelframe.Label', background='#2a3b5a', foreground='#e0e0e0')
            style.configure('TEntry', fieldbackground='#FFFFFF', foreground='#000000')
            self.root.option_add('*Menu.background', '#2a3b5a')
            self.root.option_add('*Menu.foreground', '#e0e0e0')
            ...
            style.configure('TLabel', background='#c0c0c0', foreground='#000000')
            style.configure('TButton', background='#c0c0c0', foreground='#000000')
            style.configure('TLabelframe', background='#c0c0c0', foreground='#000000')
            style.configure('TLabelframe.Label', background='#c0c0c0', foreground='#000080')
            style.configure('TEntry', fieldbackground='#ffffff', foreground='#000000')
            self.root.option_add('*Menu.background', '#c0c0c0')
    ...
    def create_log_view(self, parent):
        ...
        # Lisää oikean hiiren painike muokkaukseen
    ...
    def create_quick_controls(self, parent):
        ...
        common_modes = ['SSB', 'LSB', 'USB', 'CW', 'FM', 'AM', 'FT8', 'FT4', 'RTTY', 'FreeDV']
...
More info: https://github.com/oh3enk/hamlogger
    ...
    def check_special_input(self, event=None):
        ...
        # Pelkkä kirjainsarja (1-4 kirjainta) - vaihda mode
            ...
            mode_map = {
                ...
                'RTTY': 'RTTY', 'PSK': 'PSK', 'JT65': 'JT65', 'FREEDV': 'FreeDV'
            }
            if text in mode_map:
        ...
        if len(parts) >= 1:
            ...
            if any(char.isdigit() for char in callsign) and any(char.isalpha() for char in callsign):
                ...
                # Jos on kutsu + numeroita, tulkitaan ensimmäinen numero raportiksi
                    ...
                    # Poista WWFF comment_parts:sta
                    if wwff_found_index >= 0:
                        comment_parts = comment_parts[:wwff_found_index] + comment_parts[wwff_found_index+1:]
        ...
        if len(text.split()) >= 2:
            ...
            if len(parts) >= 3:
                if parts[1].isdigit() and parts[2].isdigit():
                    ...
                    # Poista WWFF remaining_parts:sta
                    if wwff_found_index >= 0:
                        remaining_parts = remaining_parts[:wwff_found_index] + remaining_parts[wwff_found_index+1:]
        ...
        for record in records:
            ...
            if 'CALL' in tags:
                try:
                    ...
                    # Tukee sekä QSO_DATE että DATE kenttää
                    if 'QSO_DATE' in tags:
                    ...
                    # Etsi vasta-aseman WWFF-tunnus (tukee sekä SIG_INFO että WWFF_REF)
                    their_wwff = ""
                    if 'SIG_INFO' in tags and tags.get('SIG') == 'WWFF':
                        their_wwff = tags['SIG_INFO']
                    elif 'WWFF_REF' in tags:
                        their_wwff = tags['WWFF_REF']
    ...
    def generate_adi(self):
        ...
        adi_content.append("<CREATED_TIMESTAMP:15>%s" % datetime.datetime.now(datetime.UTC).strftime('%Y%m%d %H%M%S'))
        ...
        for qso in self.log_entries:
            ...
            if qso.get('comment') and qso['comment']:
    ...
    def export_partial_log(self):
        ...
        ttk.Label(export_window, text="Valitse viennin ajankohdat (aikavälillä):").pack(pady=10)
        ...
        start_date_entry.insert(0, datetime.datetime.now(datetime.UTC).strftime('%d.%m.%Y'))
        ...
        end_date_entry.insert(0, datetime.datetime.now(datetime.UTC).strftime('%d.%m.%Y'))
    ...
    def merge_logs(self):
        ...
        try:
            ...
            default_filename = f"{self.settings['mycall']}_merged_{datetime.datetime.now(datetime.UTC).strftime('%Y%m%d_%H%M')}.adi"
            filename = filedialog.asksaveasfilename(
    ...
    def show_log_context_menu(self, event):
        """Näytä kontekstivalikko oikealla hiiren painikkeella"""
        # Etsi klikattu rivi
        index = self.log_text.index(f"@{event.x},{event.y}")
        line_num = int(index.split('.')[0])
        
        # Tarkista että rivi on datarivi (ei tyhjä)
        line_content = self.log_text.get(f"{line_num}.0", f"{line_num}.end")
        if not line_content.strip():
            return
        
        # Laske log_entries indeksi (rivit ovat käänteisessä järjestyksessä)
        entry_index = len(self.log_entries) - line_num
        if entry_index < 0 or entry_index >= len(self.log_entries):
            return
        
        # Luo kontekstivalikko
        context_menu = tk.Menu(self.root, tearoff=0)
        context_menu.add_command(label=self.texts['edit_entry'], 
                                command=lambda: self.edit_log_entry(entry_index))
        context_menu.add_command(label=self.texts['delete_entry'], 
                                command=lambda: self.delete_log_entry(entry_index))
        
        context_menu.post(event.x_root, event.y_root)
    
    def on_log_double_click(self, event):
        """Käsittele kaksoisklikkaus - avaa muokkausdialogi"""
        index = self.log_text.index(f"@{event.x},{event.y}")
        line_num = int(index.split('.')[0])
        ...
        self.edit_log_entry(entry_index)
    
    def edit_log_entry(self, index):
        """Muokkaa lokimerkintää"""
        if index < 0 or index >= len(self.log_entries):
            return
        
        entry = self.log_entries[index]
        
        edit_window = tk.Toplevel(self.root)
        edit_window.title(self.texts['edit_qso'])
        ...
        # Kutsu
        ttk.Label(edit_window, text="Kutsu:").grid(row=row, column=0, sticky=tk.W, padx=10, pady=5)
        call_var = tk.StringVar(value=entry.get('call', ''))
        ttk.Entry(edit_window, textvariable=call_var, width=20).grid(row=row, column=1, sticky=tk.W, padx=10, pady=5)
        fields['call'] = call_var
        row += 1
        
        # Bandi
        ttk.Label(edit_window, text="Bandi:").grid(row=row, column=0, sticky=tk.W, padx=10, pady=5)
        band_var = tk.StringVar(value=entry.get('band', ''))
        band_combo = ttk.Combobox(edit_window, textvariable=band_var, width=18)
        band_combo['values'] = ['160m', '80m', '60m', '40m', '30m', '20m', '17m', '15m', '12m', '10m', '6m', '2m', '70cm']
        band_combo.grid(row=row, column=1, sticky=tk.W, padx=10, pady=5)
        fields['band'] = band_var
        row += 1
        
        # Mode
        ttk.Label(edit_window, text="Mode:").grid(row=row, column=0, sticky=tk.W, padx=10, pady=5)
        mode_var = tk.StringVar(value=entry.get('mode', ''))
        mode_combo = ttk.Combobox(edit_window, textvariable=mode_var, width=18)
        mode_combo['values'] = ['SSB', 'LSB', 'USB', 'CW', 'FM', 'AM', 'FT8', 'FT4', 'RTTY', 'FreeDV']
        ...
        # RST Sent
        ttk.Label(edit_window, text="RST Lähetetty:").grid(row=row, column=0, sticky=tk.W, padx=10, pady=5)
        rst_sent_var = tk.StringVar(value=entry.get('rst_sent', ''))
        ttk.Entry(edit_window, textvariable=rst_sent_var, width=10).grid(row=row, column=1, sticky=tk.W, padx=10, pady=5)
        fields['rst_sent'] = rst_sent_var
        row += 1
        
        # RST Rcvd
        ttk.Label(edit_window, text="RST Vastaanotettu:").grid(row=row, column=0, sticky=tk.W, padx=10, pady=5)
        rst_rcvd_var = tk.StringVar(value=entry.get('rst_rcvd', ''))
        ttk.Entry(edit_window, textvariable=rst_rcvd_var, width=10).grid(row=row, column=1, sticky=tk.W, padx=10, pady=5)
        fields['rst_rcvd'] = rst_rcvd_var
        row += 1
        
        # WWFF
        ttk.Label(edit_window, text="WWFF-viite:").grid(row=row, column=0, sticky=tk.W, padx=10, pady=5)
        wwff_var = tk.StringVar(value=entry.get('their_wwff', ''))
        ttk.Entry(edit_window, textvariable=wwff_var, width=20).grid(row=row, column=1, sticky=tk.W, padx=10, pady=5)
        fields['their_wwff'] = wwff_var
        row += 1
        
        # Kommentti
        ttk.Label(edit_window, text="Kommentti:").grid(row=row, column=0, sticky=tk.W, padx=10, pady=5)
        comment_text = tk.Text(edit_window, height=4, width=40)
        comment_text.insert('1.0', entry.get('comment', ''))
        comment_text.grid(row=row, column=1, sticky=tk.W, padx=10, pady=5)
        fields['comment'] = comment_text
        row += 1
        
        def save_changes():
            # Päivitä merkintä
            entry['call'] = call_var.get().upper()
            entry['band'] = band_var.get()
            entry['mode'] = mode_var.get()
            entry['rst_sent'] = rst_sent_var.get()
            entry['rst_rcvd'] = rst_rcvd_var.get()
            entry['their_wwff'] = wwff_var.get().upper()
            entry['comment'] = comment_text.get('1.0', 'end-1c').strip()
        ...
        ttk.Button(button_frame, text="Tallenna", command=save_changes).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="Peruuta", command=edit_window.destroy).pack(side=tk.LEFT, padx=5)
    
    def delete_log_entry(self, index):
        """Poista lokimerkintä"""
        if index < 0 or index >= len(self.log_entries):
            return
        ...
        if messagebox.askyesno(self.texts['delete_entry'], 
                               f"Haluatko varmasti poistaa yhteyden {call}?"):
            del self.log_entries[index]
            self.log_modified = True
            self.refresh_log_display()
            self.update_stats()
            self.update_header()
    ...
    def refresh_log_display(self):
        """Päivitä lokinäyttö kokonaan"""
        self.log_text.delete('1.0', tk.END)
        for qso in reversed(self.log_entries):
            self.add_to_log_display(qso)
